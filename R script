...
query_groups_names <- read.csv(file="T:/Your_WorkingDirectory/query_groups_names.csv",header=TRUE)
...
for(j in 1:query_groups_names_L){
  ...
  query_group_expression <- read.csv(file=paste(queryname,".csv", sep=""),header=TRUE, row.names=1)
...
# save the results as HTML tables in your working directory
  ...
  pValue.bonf.x <- xtable(pValue.bonf,digits=4)
    print(pValue.bonf.x, type="html", file=paste(queryname,"bonf",".html",sep=""))

  pValue.holm.x <- xtable(pValue.holm,digits=4)
    print(pValue.holm.x, type="html", file=paste(queryname,"holm",".html",sep=""))

  pValue.fdr.x <- xtable(pValue.fdr,digits=4)
    print(pValue.fdr.x, type="html", file=paste(queryname,"fdr",".html",sep=""))
